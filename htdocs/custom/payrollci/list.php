<?php
/* ============================================================================
 * PayrollCI v3 - Liste des bulletins de paie - Intégration Dolibarr
...
if ($massaction == 'validate' && !empty($toselect)) {
    foreach ($toselect as $sid) {
        $obj = new Payslip($db);
        $obj->fetch($sid);
        if ($obj->status == Payslip::STATUS_DRAFT) {
            $obj->validate($user);
        }
    }
...
if ($user->rights->payrollci->creer || $user->admin) {
    $newcardbutton = '<a class="butAction" href="card.php?action=create">Nouveau bulletin</a>';
}

print load_fiche_titre('Bulletins de Paie', $newcardbutton, 'payrollci@payrollci');

// ── Résumé (si filtré par période) ──
if ($search_month > 0 || $search_year > 0) {
    ...
    print '<tr class="liste_titre"><td colspan="4"><b>Résumé - '.$periodLabel.'</b></td></tr>';
